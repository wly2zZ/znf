					...
					success: async function(res) {
						if (res.confirm) {
							...
							for (let i = 0; i < _this.orderGoods.length; i++) {
                                ...
                                // 当前商品是否相同表
                                if(_this.orderGoods[i].tablename == `shangpinxinxi`){
                                     // 获取数量 
                                     let res = await _this.$api.info('shangpinxinxi', _this.orderGoods[i].goodid);
                                     // 单次限次
						...
						res.data.selected =  res.data.selected  + "," + _this.orderGoods[i].activeSeat
                                         ...
                                         // 库存充足则减少库存数量
                                         await _this.$api.update(`shangpinxinxi`,  res.data);
                                     }
                                }
								let order =  {
